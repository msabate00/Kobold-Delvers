    ...
    int screenX, int screenY, Material m, int r)
{
    ...
    if (app->scenes->CurrentId() != SceneId::SCENE_LEVEL4 && app->scenes->CurrentId() != SceneId::SCENE_LEVEL8 && app->scenes->CurrentId() != SceneId::SCENE_LEVEL12) {
    ...
    if (engine.app && engine.app->audio) {
        if (!paintInstance) {
            ...
            engine.app->audio->SetVoicePosition("paint", paintInstance, cx, 0);
        }
    }
}
